...
public class MusicPFA {
   ...
   */
  public MusicPFA(File file, int trackNum) throws Exception {

    this.tickMap = new HashMap<Long, State>();
    Sequence seq = MidiSystem.getSequence(file);

    this.tempo = readTempo(seq);
  ...
  /**
   * Generates a sequence of 500 states from the transitionProbbilityMap
   * Selects a random starting state
   */
  public StateSequence generateStateSequence(int n) {
    // select a random state
    Set<State> stateSet = this.transitionProbabilityMap.keySet();
    State[] stateArr = stateSet.toArray(new State[stateSet.size()]);
    Random rand = new Random();
    int randomInt = rand.nextInt(stateArr.length);
    State startState = stateArr[randomInt];
    // use the other method
    return generateStateSequence(startState, n);
  }

  /**
   * Generates a sequence of n states from the transitionProbbilityMap
   * Uses the specified starting state
   */
  public StateSequence generateStateSequence(State start, int n) {
    StateSequence seq = new StateSequence();
    seq.add(start);
    Random rand = new Random();
    for(int i = 0; i < n; i++) {
      // add a state from the transitionProbabilityMap
      State prev = seq.getLast();
      ArrayList<State> possibleNextStates = this.transitionProbabilityMap.get(prev);
      if(possibleNextStates.size() == 0) continue;
      int randomStateIndex = rand.nextInt(possibleNextStates.size());
      seq.add(possibleNextStates.get(randomStateIndex));
    }
    System.out.println(seq.toString());
    return seq;
  }

  public static void createMIDI(StateSequence seq, String outFile) throws IOException {
    MidiFile mf = new MidiFile();
    for(State s : seq.stateSequence) {
      // turn on the notes in the sequence
      for(Note n : s.notes) {
        mf.noteOn(0, n.key, 127);
      }
      // turn off the first note with delta MidiFile.CROTCHET
      mf.noteOff(MidiFile.QUAVER, s.notes.get(0).key);
      // turn off the rest of the notes
      for(int i = 1; i < s.notes.size(); i++) {
        mf.noteOff(0, s.notes.get(i).key);
      }
    }
    mf.writeToFile(outFile);
  }
...
class StateSequence {
  ...
  public ArrayList<State> stateSequence;
  ...
  public State getLast() {
    return this.stateSequence.get(this.stateSequence.size() - 1);
  }

  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("State Sequence:");
    for(State s : this.stateSequence) {
      sb.append("#\n");
